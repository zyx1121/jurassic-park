...
- Player feedback uses a native-resolution uGUI HUD and contextual world selection. `build_hud_assets` and `build_selection_assets` create persistent settings; scene builders wire both through `BuildFeedbackHud`. Register UI geometry with `WorldInputBlockers` rather than relying on EventSystem state inside input callbacks. Keep WASD movement and show real gameplay state, not placeholder hunger or repaired-boat progress.
