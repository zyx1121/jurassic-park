...
namespace JurassicPark.Presentation
{
    ...
    public sealed class SimulationRuntime
    {
        ...
        public static SimulationRuntime Build(SimulationSettingsAsset settings, MapDefinitionAsset mapAsset, EntityCatalogAsset catalogAsset, ScenarioAsset scenario, MatchRulesAsset matchRules = null)
        {
            ...
            runtime.Spawner = new EntitySpawner(runtime.World, runtime.Map, runtime.Catalog, runtime.Logistics, runtime.Vitals, runtime.Structures);
            if (matchRules != null)
            {
                runtime.Match = new MatchFlow(runtime.World, matchRules.ToRules(), runtime.Seats, runtime.Map, runtime.Catalog, runtime.Spawner);
                runtime.Router.Register(CommandKind.ChooseMatch, new ChooseMatchCommandHandler(runtime.Match));
                runtime.Router.Register(CommandKind.Board, new BoardCommandHandler(runtime.Tasks, runtime.Match));
            }
        ...
        private void Place(EntityCatalogAsset catalogAsset, ScenarioAsset.Placement placement, List<string> problems)
        {
            ...
            Entity entity = Spawner.Spawn(entry.id, new SeatId(placement.ownerSeat), new Cell(placement.cellX, placement.cellY), out string problem);
            if (entity == null)
            {
            ...
            for (int i = 0; i < placement.stock.Length; i++)
                if (Logistics.Seed(entity.Id, placement.stock[i].resource, placement.stock[i].amount) < placement.stock[i].amount)
                    problems.Add($"'{entry.id}' at ({placement.cellX}, {placement.cellY}) cannot hold its starting stock of {placement.stock[i].amount} {placement.stock[i].resource}.");
        }
    }
}
