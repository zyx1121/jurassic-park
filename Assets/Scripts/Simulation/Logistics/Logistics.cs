...
namespace JurassicPark.Simulation
{
    ...
    public sealed class Logistics : ISimSystem
    {
        ...
        public int Gather(EntityId nodeHolder, EntityId into, int amount, Reservation stock, Reservation room)
        {
            ...
            int fromNode = IsLive(stock) ? Math.Min(stock.Amount, node.Remaining) : node.Unreserved;
            int intoRoom = IsLive(room) ? room.Amount + Math.Max(0, target.FreeCapacity) : target.FreeCapacity;
            int moved = Math.Min(amount, Math.Min(fromNode, intoRoom));
            if (moved < 1) return 0;
        ...
        public int Transfer(EntityId from, EntityId to, string resource, int amount, Reservation withdrawal, Reservation deposit, TaskId onBehalfOf = default)
        {
            ...
            bool ownsWithdrawal = IsLive(withdrawal) && withdrawal.Holder == from && withdrawal.Resource == resource;
            int takeable = AvailableIn(from, resource, ownsWithdrawal ? withdrawal.Owner : onBehalfOf);
            int room = IsLive(deposit) && deposit.Holder == to ? deposit.Amount + Math.Max(0, target.FreeCapacity) : target.FreeCapacity;
            int moved = Math.Min(amount, Math.Min(takeable, room));
            if (moved < 1) return 0;

            if (ownsWithdrawal) Settle(withdrawal, moved);
            Settle(deposit, moved);
        ...
        /// <summary>Destroys goods on purpose, for example materials built into a wall. Counted, so the conservation check still balances.</summary>
        public int Consume(EntityId from, string resource, int amount)
        {
            if (amount < 1 || !containers.TryGetValue(from, out Container source)) return 0;
            int used = Math.Min(amount, source.AmountOf(resource));
            if (used < 1) return 0;
        ...
        /// <summary>Everything of one resource that exists anywhere: node stock plus goods in containers. With <see cref="ConsumedTotal"/> it is constant for a match.</summary>
