...
| 實作（2026-09-18，#107） | `Knowledge` 每隊一份 explored / visible 遮罩，revision 只在變動時前進；快照過濾規則：自己與盟友的東西永遠給，別席位的單位只在可見格給，別席位的建築與無主物（樹、掉落堆）探索過就給；`jp.fog` 只在 revision 變動時送整份遮罩（protocol 4）；`FogView` 每格一片頂點 alpha 的霧。對霧裡的東西下指令等於走過去（照原版） |
