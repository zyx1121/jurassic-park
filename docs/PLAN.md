# Jurassic Park：v1 設計計畫

v1，2026-09-17。本文是專案唯一的設計來源。2026-09-17 repo 已重置：舊原型與未驗證的 Infrastructure 切片保存在 `legacy` 分支作為經驗參考，`main` 從本文重新實作。本文不是已實作功能清單。

# 前言

> [!NOTE]
> 建議閱讀順序：
> 1. ***[本文件：v1 設計計畫](./PLAN.md)***：決策狀態、系統邊界、主流程、施工順序與 milestone。
> 2. [原始提案](./PROPOSAL.md)：課程背景與日期；玩法內容已被本文取代。
> 3. [美術方向](./ART_DIRECTION.md)：2026-09-17 定案的可讀 RTS 叢林風格。
> 4. [README](../README.md)：專案簡介與啟動方式。
...
| 已確認（2026-09-17） | 施工與退款：取消時未消耗材料留在搬運者或落地，已消耗材料不退款。 |
...
| 已確認（2026-09-17） | 多人傳輸：host authority 執行模擬；client 只把 Command 送到 host（ServerRpc），host 回傳接受／拒絕與狀態快照；client 不跑模擬、不預測。詳見 §2.4。 |
| 已確認（2026-09-17） | 對局流程全部照原版 6.3，採用原版自帶的 30 分鐘模式（1800 秒倒數 + 300 秒撤離窗口），時間表不縮放，數值來源見 §9。 |
...
| 待決策 | 席位數上限（原版 7 席，第一版只驗 2 人 + 1 AI）；盟友借用倉庫是否計入預約優先權；玩家離線轉 AI 的觸發條件。 |
...
模擬核心是純 C#，Command 是小型可序列化 struct（CommandId、Owner、ActorId、Kind、Cell、TargetId），這決定了傳輸模型：
...
host：驗證 Owner 與席位綁定 → World.Submit → World.Tick
        ↓
CommandResult 回發送者（Accepted / Reason）
        ↓
狀態快照（實體位置、容器數量、占地版本、Task 狀態）廣播所有 client
        ↓
client 只做渲染與 GUI，不跑模擬
```

| 規則 | 內容 |
|---|---|
| 權威 | 只有 host 跑 `Tick`；模擬層只問「我是不是 authority」，由網路層注入答案，沒有 NetworkManager 時為單機 |
...
| 不做 | client 端預測、deterministic lockstep、client 端模擬副本 |

單機模式是「host 自己是唯一 client」，走同一條路徑；所有輸入都經過席位路由，不直接呼叫模擬。這一層在 M1 完成兩席位身分時一起接上，不留到最後。
...
解碼結果（2026-09-19，`docs/original/map_layout.json`，工具 `tools/original_map_layout.py`）：可玩區為 128 格中的 116 × 116（x 6..121、y 4..119），外圈是不可玩邊界；水 1,683 格、崖層 0–7、地物 6,849 筆（樹 3,660、植物 1,718、岩石 492、其他 979）；12 個 `-base` 營地矩形、10 個撤離矩形、7 名倖存者集結在中央 `Vo` 矩形。**待確認**：原版把最常見的兩種樹 `ZTtw`/`ZTtc`（3,120 棵）的 pathing 改成 `2x2Unflyable.tga`，依 Warcraft 慣例只擋空中不擋地面；若屬實，上表「樹木擋路」在原版只對其他樹成立，新地圖設計前要先決定樹是否擋地面。
...
> [!IMPORTANT]
> 截至 2026-09-17，`main` 沒有任何遊戲程式。以下 milestone 對應 GitHub milestone 與課程日期，逐項以 issue 追蹤；完成一項才在此更新狀態。
...
`legacy` 分支保存 2026-09-14 到 2026-09-17 的原型：程序島嶼、WASD 生存循環、NGO 直連、digitized sprite 管線，以及未驗證的 Infrastructure 切片。它是經驗來源，不是程式來源：可以回頭查做法與踩過的坑（場景建置順序、VolumeProfile subasset、LFS 規則、檔名等於類名），但不整批搬回程式。美術處理腳本保留在 `tools/`，因為管線延續。
...
> [!WARNING]
> 官方／社群地圖樣本可能有版本差異；不匯入魔獸模型、貼圖、音效、UI 皮膚或地圖腳本作為新遊戲資產。只借鑑玩法、系統關係與數值。

## 9. 對局流程與內容基線（照原版 6.3）

> [!NOTE]
> 本節的數值以原版 6.3 地圖物件資料解析結果為唯一來源，完整清單在 [ORIGINAL_CONTENT.md](./ORIGINAL_CONTENT.md)：162 個單位、98 個物品、207 個技能、70 個科技、10 個可破壞物。腳本是混淆過的純 JASS；日夜、視野與 31 組生成計時器已於 2026-09-17 深挖補齊，剩夜視鏡加成型態與 t=1800 秒批次的臨界時序標 uncertain。

### 9.1 對局流程

| 項目 | 規則 |
|---|---|
| 總長 | 採用原版自帶的 **30 分鐘模式**（生存倒數 1800 秒）；原版另有 45／60 分鐘模式，先不做。時間表直接用原版數值，不需要縮放 |
| 開局 | 照原版：出生、初始物資、`-base` 12 個推薦營地由玩家自選 |
| 日夜 | 照原版：一天 480 秒（引擎預設，腳本未改流速），開局固定 17:30，撤離時凍結在 03:00；17:00 到 07:00 禁止建造營火 |
| 波次與事件 | 照原版：31 組週期計時器分兩層，難度選定時立即開一批，另有 t=600 到 2900 秒的時間軸陸續加開或替換；30 分鐘模式只到 t≈1800 秒的批次。照抄隨機性，完整表在 `ORIGINAL_CONTENT.md` |
| 撤離 | 照原版：倒數歸零後直升機從候選區選點降落，停留 **300 秒**，登機才算勝利，逾時戰敗；不做修船 |
| 勝負與難度 | 照原版：撤離成功／全滅／錯過直升機；6 級難度（簡單到侏儸紀公園 II），開局 20 秒選擇視窗，逾時自動選 30 分鐘 |
| 席位 | 原版 7 個倖存者席位 + 5 個恐龍／昆蟲電腦陣營；程式上限照原版 7，第一版驗 2 人 + 1 AI |
...
| 光源 | 照原版：營火類單位的視野是自身靜態日／夜半徑（400 或 500），沒有光環；夜視鏡 +700 僅夜間，望遠鏡 +400 全天 |
...
| 實作（2026-09-18，#107） | `Knowledge` 每隊一份 explored / visible 遮罩，revision 只在變動時前進；快照過濾規則：自己與盟友的東西永遠即時給，其他一切只在可見格即時給；別席位的建築與無主物（樹、掉落堆）離開視野後由 host 的 `SnapshotMemory` 送該隊最後一次看到的快照（標 `Remembered`），再次看到空格才遺忘，單位不記憶；`jp.fog` 只在 revision 變動時送整份遮罩（protocol 4）；`FogView` 每格一片頂點 alpha 的霧。對霧裡或只剩記憶的東西下指令等於走過去（照原版） |
...
> - [原始提案](./PROPOSAL.md)：課程背景與日期。[美術方向](./ART_DIRECTION.md)：現行視覺規格。
