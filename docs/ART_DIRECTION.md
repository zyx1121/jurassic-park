...
| Decision | Value |
|---|---|
| Camera | **Perspective, as the original**: pitch 56 degrees, 70 degree horizontal lens (43 vertical at 16:9), 26 m from the focus (the original's 1650 units at 128 units per cell), yaw fixed at 45 degrees so the cell grid runs diagonally on screen, pan (screen-relative) and zoom only. Chosen by the owner on 2026-09-18 with the rule that guides every visual choice: it must look like playing the original, with better code, performance and art, and nothing changed in mechanics or feel. The lean of upright things near the screen edge is the original's look, not a defect. An orthographic version was tried on 2026-09-17 and rejected on 2026-09-18: it reads as a flat tile map. Visual choices like this are the owner's; engineering proposes with comparison captures. Zoom 14 to 44 m |
